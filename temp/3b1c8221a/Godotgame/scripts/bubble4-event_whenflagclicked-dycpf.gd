...
func _init_after_ready():
	...
	while true:
		...
		secs = correctur.ms(op.rand("1.1","3"), "float", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "wait (!) secs")
		await get_tree().create_timer(secs).timeout

		main.move_child(object, 2)

		costume = correctur.ms("bubble1", "string", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "switch costume to (!)")
		...
		condition = correctur.ms(op.equal(op.rand("1","2"),"1"), "bool", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "if <!> then: {} else: {}")
		if condition:
			...
			if str(goTo) == "_mouse_":
				...
				if target_node and target_node is Node2D:
					...
					correctur.print_not_existing("res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "go to ()", str(goTo))
		...
		size = correctur.ms("100", "float", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "set size to (!)")
		if size < 0:
			size = 0
		...
		condition = correctur.ms(op.greater((object.position.y * -1),"165"), "bool", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "repeat until <!>: {}")
		while not(condition):
			condition = correctur.ms(op.greater((object.position.y * -1),"165"), "bool", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "repeat until <!>: {}")

			times = correctur.ms("20", "float", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "repeat (): {}")
			for _k in int(times):

				size = correctur.ms("1", "float", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "change size by (!)")
				if (object.size + size) < 0:
					size = 0
				animation.scale.x += size / 100 * object.size_x * object.stretch.x / 100
				animation.scale.y += size / 100 * object.stretch.y / 100 

				Ynew = correctur.ms("2", "float", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "change y by (!)")
				object.position.y -= Ynew

				await get_tree().process_frame

			times = correctur.ms("10", "float", "res://scripts/bubble4-event_whenflagclicked-dycpf.gd", "repeat (): {}")
			for _k in int(times):
